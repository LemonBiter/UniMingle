...
@section('content_header')
    ...
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            ...
            <button type="button" onclick="location.href = '{{ route('admin.events.create') }}';"
        ...
        @if(count($events))
            ...
            @foreach($events as $l)
                <div class="col-md-3 item{{ $l->id }}">
                    <a href="{{ route('admin.events.show',$l->id) }}">
                        @if($l->coverImage)
                            <img src="{{ $l->coverImage->url }}" alt="unimingle-event-{{ $l->id }}" class="img-responsive" style="width: 278px; height: 278px;">
                        @else
                            <img src="{{ asset('images/event-default-cover.jpg') }}" alt="unimingle-event-default-cover" class="img-responsive" style="width: 278px; height: 278px;">
                        @endif
                    </a>
                    <div class="card">
                        <div class="card-body">
                            <a href="{{ route('admin.events.show',$l->id) }}">
                                <h5 class="card-title" title="{{ $l->title }}">{{ $l->getTitle() }}</h5>
                            </a>
                            <p>
                            ...
                            <button class="btn btn-success btn-rounded waves-effect waves-light m-t-10"
                                    onclick="location.href = '{{ route('admin.events.edit',$l->id) }}';">Edit
                            </button>
                            <button class="btn btn-danger btn-rounded waves-effect waves-light m-t-10 delete-event"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="tooltipmodel">Are you sure you want to delete this event</h4>
        ...
        $(document).on('click', '.delete-event', function () {
            ...
            swal({
                ...
                if (isConfirm.value) {
                    ...
                    $.ajax({
                        type: 'DELETE',
                        url: location.origin + '/admin/events/' + id,
                        data: {
